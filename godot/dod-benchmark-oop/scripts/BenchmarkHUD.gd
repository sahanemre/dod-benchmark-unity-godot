...
## Benchmark kontrol paneli — Control node'lari ile olusturulur.
...
func setup(entity_counts: Array[int]) -> void:
	...
	var title := Label.new()
	title.text = "DOD Benchmark — Godot OOP"
...
func _update_selection() -> void:
	for i in range(_entity_buttons.size()):
		if i == _selected_index:
			_entity_buttons[i].add_theme_color_override("font_color", Color.GREEN)
		else:
